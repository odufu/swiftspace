...
import '../../../auth/presentation/pages/professional_application_screen.dart';
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              isApplicationIncomplete
                  ? 'Please provide your professional credentials and identity verification to access the ${profile?.role.portalName}.'
                  : 'A Super Admin is currently reviewing your ${profile?.role.displayName.toLowerCase() ?? 'professional'} profile. You\'ll gain access once approved.',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 16,
                ...
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const ProfessionalApplicationScreen(),
                      ),
                    );
                  },
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Complete 7 more sales to unlock the Platinum Badge and boost buyer trust.',
                    style: TextStyle(color: Colors.grey[600], fontSize: 12),
                  ),
                ],
              ),
            ...
            Row(
              children: [
                Expanded(
                  child: Consumer<PropertyProvider>(
                    builder: (context, provider, _) {
                      final totalViews = provider.myProperties.fold(
                        0,
                ...
                Expanded(
                  child: Consumer<PropertyProvider>(
                    builder: (context, provider, _) {
                      final totalInteractions = provider.myProperties.fold(
                        0,
                        (sum, p) => sum + p.favoritesCount + p.videoViewsCount,
                      );
                      return _buildMetricCard(
                        context,
                ...
                Expanded(
                  child: Consumer<PropertyProvider>(
                    builder: (context, provider, _) {
                      ...
                      final hotLeads = provider.myProperties.fold(
                        0,
                        (sum, p) => sum + (p.favoritesCount > 0 ? 1 : 0),
                      );
                      return _buildMetricCard(
                        context,
                        LucideIcons.users,
...
class _AgentAccountingTabState extends State<_AgentAccountingTab> {
  ...
  Widget build(BuildContext context) {
    ...
    final pendingEscrow = transactions
        ...
        .fold<double>(0, (sum, t) => sum + t.amount);
    final overdueCount = transactions
        .where((t) => t.state == EscrowState.overdue)
        .length;
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '$overdueCount client(s) have defaulted on installments.',
                          style: const TextStyle(
                            fontSize: 12,
                            color: Colors.red,
                          ),
                  ...
                  TextButton(
                    onPressed: () {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text(
                            'Automated reminders sent to defaulting clients!',
                          ),
          ...
          ...transactions.expand(
            (tx) => tx.invoices.map(
              (inv) => _buildInvoiceItem(theme, isDark, inv, tx),
            ),
          ),
          ...
          ...transactions.expand(
            (tx) => tx.installments
                .where((i) => !i.isPaid)
                .map((inst) => _buildInstallmentItem(theme, isDark, inst, tx)),
          ),
      ...
      child: Row(
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: (inv.type == 'Receipt' ? Colors.green : Colors.blue)
                  .withValues(alpha: 0.1),
              shape: BoxShape.circle,
            ),
            child: Icon(
          ...
          Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                  fontWeight: FontWeight.bold,
                  color: inst.isOverdue ? Colors.red : null,
                ),
              ),
              Text(
                inst.isOverdue
                    ? 'OVERDUE'
                    : 'Due: ${inst.dueDate.day}/${inst.dueDate.month}',
                style: TextStyle(
                  color: inst.isOverdue ? Colors.red : Colors.grey[500],
                  fontSize: 11,
                  fontWeight: inst.isOverdue ? FontWeight.bold : null,
                ),
              ),
            ],
          ),
...
class _AgentProfileTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          '${authProvider.profile?.role.displayName ?? 'Agent'} Settings',
          style: const TextStyle(fontWeight: FontWeight.bold),
        ),
        centerTitle: false,
        automaticallyImplyLeading: false,
        ...
        child: ConstrainedBox(
          ...
          child: ListView(
            ...
            children: [
              ...
              const SizedBox(height: 24),
              Text(
                '${authProvider.profile?.role.displayName ?? 'Agent'} Preferences',
                style: const TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.bold,
                  color: Colors.grey,
